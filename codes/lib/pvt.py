...
class CFM(nn.Module):
    ...
    def forward(self, x1, x2, x3):
        x1_1 = x1
        x2_1 = self.conv_upsample1(self.interpolate(x1)) * x2
        x3_1 = self.conv_upsample2(self.interpolate(self.interpolate(x1))) \
               * self.conv_upsample3(self.interpolate(x2)) * x3

        x2_2 = torch.cat((x2_1, self.conv_upsample4(self.interpolate(x1_1))), 1)
        x2_2 = self.conv_concat2(x2_2)

        x3_2 = torch.cat((x3_1, self.conv_upsample5(self.interpolate(x2_2))), 1)
...
class PolypPVT(nn.Module):
    def __init__(self, channel=32):
        ...
        self.ssp = SSP_MatchingNet(refine=True)
